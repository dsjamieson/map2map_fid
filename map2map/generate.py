...
def generate(args):
    ...
    if not args.no_dis :
        d2d_model = import_attr("vnet.StyledVNet", models, callback_at=args.callback_at)
        d2d_model = d2d_model(sum(chan), sum(chan),
        ...
        with torch.no_grad():
            for i, data in enumerate(generate_loader):
                ...
                output = narrow_cast(d2d_model(input))[0]
    ...
    if not args.no_vel :
        v2v_model = import_attr("vnet.StyledVNet", models, callback_at=args.callback_at)
        v2v_model = v2v_model(sum(chan), sum(chan),
                      scale_factor=args.scale_factor, **args.misc_kwargs)
        v2v_model.to(device)
        ...
        with torch.no_grad():
            for i, data in enumerate(generate_loader):
                ...
                input = data['input']
                input = input.to(device, non_blocking=True)
                output = narrow_cast(v2v_model(input))[0]
